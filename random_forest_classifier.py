...
X_train = train_df_copy[
    [i for i in train_df_copy.columns if i not in ["num_cycles", "id"]]
]
...
# test set
X_test = test_df[[i for i in test_df.columns if i not in ["num_cycles", "id"]]]
Y_test = test_df["num_cycles"]

y_hat = rf_model.predict(X_test)
# print(f"test RMSE: {root_mean_squared_error(Y_test, y_hat)}")
print(f"test accuracy: {accuracy_score(Y_test, y_hat)}")
print(f"test f1: {f1_score(Y_test, y_hat)}")
print(f"test recall: {recall_score(Y_test, y_hat)}")
print(f"test precision: {precision_score(Y_test, y_hat)}")
